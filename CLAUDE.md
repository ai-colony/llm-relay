...
- `GET /metrics` sets the `llm_relay_*` prompt-queue gauges via `setGauge` and renders everything — gauges, counters, histograms — through the single `renderMetrics()` in `src/lib/metrics.ts`: `http_requests_total`/`http_request_duration_seconds` (recorded by the `httpMetrics` middleware in `src/hono/httpMetrics.ts`, mounted in `src/hono/index.ts`), `openai_requests_total`/`openai_request_duration_seconds` (worker path), `openai_chat_requests_total`/`openai_chat_request_duration_seconds` (`/chat/completions`), and `callback_deliveries_total`. The `llm_relay_prompts_*` metrics are gauges, not counters — they are point-in-time DB counts and decrease on `DELETE /prompt/purge`.
...
`src/lib/index.ts` is the barrel; it re-exports `config`, `logger`, `checkOpenAI`, `executeOpenAIPrompt`, `streamChatCompletion`, `getModelInfo`, `isCallbackUrlAllowed`, `checkCallbackAvailability`, `incCounter`, `setGauge`, `observeHistogram`, `recordUpstreamMetrics`, `renderMetrics`, and the chat schema types. Add new lib exports there when creating new modules, and import through `@lib` rather than reaching past it.

- `src/lib/openAI.ts` — OpenAI SDK streaming integration; lazy-resolves the model name once on first use; tracks reasoning vs response tokens separately and calculates tokens-per-second metrics. The "Sending prompt" log includes a `sizes` field with character counts for system and user prompts. Exports `checkOpenAI` (used by `GET /health`), `getModelInfo` (used by `GET /status`), `executeOpenAIPrompt` (used by the worker), and `streamChatCompletion` (used by `POST /chat/completions`) — all re-exported from the `@lib` barrel. Request/message schemas live in `src/lib/chatSchemas.ts` and are re-exported by `src/hono/chat/schemas.ts`, so `@lib` never depends on the HTTP layer.
...
- `src/lib/logger.ts` — Pino logger; use structured fields, not string interpolation. Every log call includes a `component` field (`'server'`, `'http'`, `'worker'`, `'callback'`, `'openai'`, `'chat'`) to identify the source layer.
...
SQLite via Drizzle ORM (`drizzle-orm/node-sqlite`) using the Node.js built-in `node:sqlite` module. Schema is defined in `src/db/schema.ts`, which is also the single source of truth for the prompt-status enum — `PROMPT_STATUSES` backs the `PromptStatus` type, the Drizzle column, the Zod enum in `prompt/schemas.ts`, and the OpenAPI enum. The client is reached as `database.client` / `database.schema`. Schema changes are **not** auto-applied — run `npm run drizzle:push` (dev) or generate+migrate (prod) after editing the schema. The `prompts` table enforces a unique index on `(clientName, requestId)` — duplicate pairs are rejected at the DB layer.

### Key patterns

- **Worker loop**: `src/index.ts` schedules each tick with a 100 ms `setTimeout` and runs `processQueuedPrompts` and `processCallbackPendingPrompts` concurrently via `Promise.all` — they touch disjoint rows, so a slow callback batch must not delay picking up queued prompts. Each tick fetches up to `WORKER_CONCURRENCY` queued prompts (lowest `priority` first, FIFO on ties), marks them all `in_progress`, then processes them concurrently. Up to 50 callbacks are delivered per tick (FIFO), at most 10 in flight.
...
- **Model resolution**: `getModelInfo()` in `openAI.ts` queries the upstream API and caches the promise for `OPENAI_MODEL_CACHE_TTL_SECONDS` (default 60s), after which the next call re-queries. The cache timestamp is stamped at promise _creation_, not on resolution, so concurrent callers racing the first lookup share one in-flight request instead of each firing their own — this lets `/status` and the model name used for requests self-heal after the backend restarts with a different model, without restarting the relay. It selects by `OPENAI_MODEL` env var or falls back to the first available model.
