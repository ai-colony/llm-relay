...
- `src/lib/config.ts` — environment variables parsed with `env-var`; see `.env.example` for all options (`PORT`, `LOG_LEVEL`, `DATABASE_FILENAME`, `GENERATIVE_URL/MODEL/KEY`, `EMBEDDING_URL/MODEL/KEY`, `UPSTREAM_TIMEOUT`, `UPSTREAM_MAX_RETRY_COUNT`, `UPSTREAM_MODEL_CACHE_TTL_SECONDS`, `WORKER_CONCURRENCY`, `CALLBACK_URL_ALLOWLIST`, `CALLBACK_RETRY_TTL_HOURS`, `CALLBACK_HMAC_SECRET`). `config.embedding` is `EmbeddingConfig | undefined`, and **that union is what every "is embedding enabled" guard keys off**. Note `asUrlString()` throws on an empty value, hence the raw `.asString()` presence check before it.
