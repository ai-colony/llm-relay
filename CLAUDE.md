...
- `POST /prompt/add` detects duplicate `(clientName, requestId)` pairs by catching the SQLite unique-constraint error directly — it checks `error.code === 'ERR_SQLITE_ERROR' && error.errcode === SQLITE_CONSTRAINT_UNIQUE` (both exported from `@db`, defined in `src/db/errors.ts`) rather than pre-querying for existence, then returns a `409`.
...
`src/lib/index.ts` is the barrel; it re-exports `config`, `logger`, `checkOpenAI`, `executeOpenAIPrompt`, `getModelInfo`, `isCallbackUrlAllowed`, `checkCallbackAvailability`, `incCounter`, `observeHistogram`, and `renderMetrics`. Add new lib exports there when creating new modules.

- `src/lib/openAI.ts` — OpenAI SDK streaming integration; lazy-resolves the model name once on first use; tracks reasoning vs response tokens separately and calculates tokens-per-second metrics. The "Sending prompt" log includes a `sizes` field with character counts for system and user prompts. Exports `checkOpenAI` (used by `GET /health`), `getModelInfo` (used by `GET /status`), and `executeOpenAIPrompt` (used by the worker) — all three re-exported from the `@lib` barrel — plus `streamChatCompletion` (used by `POST /chat/completions` via a direct relative import, not re-exported from the barrel).
