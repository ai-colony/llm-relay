...
| `GENERATIVE_REASONING_EFFORT`      | `none`                     | Reasoning/thinking level asked of the generative backend, sent as `reasoning_effort` on every completion. `none` disables thinking — the request-level equivalent of llama.cpp's `--reasoning off`, and the only way to say it to a hosted backend. `minimal`/`low`/`medium`/`high` ask for it; `default` omits the parameter entirely and lets the backend decide. |
